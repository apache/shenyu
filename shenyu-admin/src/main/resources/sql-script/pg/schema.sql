...
ELSE
	...
	PERFORM public.dblink_exec('init_conn',  'INSERT INTO "plugin" VALUES (''' || '6' || ''', ''' || 'dubbo' || ''', ''' || '{"register":"zookeeper://localhost:2181","multiSelectorHandle":"1","threadpool":"fixed","corethreads":200,"threads":200,"queues":0}' || ''', ''' || 'Proxy' || ''', 310, 0, ''' || '2018-06-23 10:26:41' || ''', ''' || '2018-06-11 10:11:47' || ''');');
...
ELSE
    ...
    PERFORM public.dblink_exec('init_conn',  'INSERT  INTO "plugin_handle" ( plugin_id ,  field ,  label ,  data_type ,  type ,  sort ,  ext_obj ) VALUES (''' || '6' || ''', ''' || 'threadpool' || ''', ''' || 'threadpool' || ''', ''' || '3' || ''', ''' || '3' || ''', ''' || '0' || ''', ''' || '{"required":"0","defaultValue":"fixed","placeholder":"threadpool","rule":""}' || ''');');
    PERFORM public.dblink_exec('init_conn',  'INSERT  INTO "plugin_handle" ( plugin_id ,  field ,  label ,  data_type ,  type ,  sort ,  ext_obj ) VALUES (''' || '6' || ''', ''' || 'corethreads' || ''', ''' || 'corethreads' || ''', ''' || '1' || ''', ''' || '3' || ''', ''' || '0' || ''', ''' || '{"required":"0","defaultValue":"200","placeholder":"corethreads","rule":""}' || ''');');
    PERFORM public.dblink_exec('init_conn',  'INSERT  INTO "plugin_handle" ( plugin_id ,  field ,  label ,  data_type ,  type ,  sort ,  ext_obj ) VALUES (''' || '6' || ''', ''' || 'threads' || ''', ''' || 'threads' || ''', ''' || '1' || ''', ''' || '3' || ''', ''' || '0' || ''', ''' || '{"required":"0","defaultValue":"200","placeholder":"threads","rule":""}' || ''');');
