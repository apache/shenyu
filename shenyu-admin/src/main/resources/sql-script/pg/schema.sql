...
ELSE
	...
	PERFORM public.dblink_exec('init_conn',  'INSERT  INTO "plugin_handle" ( plugin_id , field , label , data_type , type , sort , ext_obj ) VALUES (''' || '124' || ''', ''' || '27' || ''', ''' || 'generalContextType' || ''', ''' || 'generalContextType' || ''', 3, 2, 1, ''' || '{"required":"1","defaultValue":"addRpcContext","rule":""}' || ''', ''' || '2021-07-18 22:52:20' || ''', ''' || '2021-07-18 22:59:57' || ''');');
...
ELSE
    ...
    PERFORM public.dblink_exec('init_conn',  'INSERT INTO "shenyu_dict" ( type ,  dict_code ,  dict_name ,  dict_value ,  "desc" ,  sort ,  enabled ) VALUES (''' || 'generalContextType' || ''', ''' || 'RPC_CONTEXT_TYPE' || ''', ''' || 'addRpcContext' || ''', ''' || 'addRpcContext' || ''', ''' || 'addRpcContext' || ''', 1, 1, ''' || '2021-11-24 14:21:58' || ''', ''' || '2021-11-24 14:21:58' || ''');');
	PERFORM public.dblink_exec('init_conn',  'INSERT INTO "shenyu_dict" ( type ,  dict_code ,  dict_name ,  dict_value ,  "desc" ,  sort ,  enabled ) VALUES ( ''' || 'generalContextType' || ''', ''' || 'RPC_CONTEXT_TYPE' || ''', ''' || 'transmitHeaderToRpcContext' || ''', ''' || 'transmitHeaderToRpcContext' || ''', ''' || 'transmitHeaderToRpcContext' || ''', 0, 1, ''' || '2021-11-24 14:21:32' || ''', ''' || '2021-11-24 14:21:32' || ''');');
