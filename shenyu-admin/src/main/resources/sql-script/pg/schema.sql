...
ELSE
    ...
    PERFORM public.dblink_exec('init_conn',  'INSERT  INTO  shenyu_dict  ( type , dict_code ,  dict_name ,  dict_value ,  "desc" ,  sort ,  enabled ) VALUES (''' || 'operator' || ''', ''' || 'OPERATOR' || ''', ''' || 'starts' || ''', ''' || 'starts' || ''', ''' || 'starts' || ''', 7, 1);');
    PERFORM public.dblink_exec('init_conn',  'INSERT  INTO  shenyu_dict  ( type , dict_code ,  dict_name ,  dict_value ,  "desc" ,  sort ,  enabled ) VALUES (''' || 'operator' || ''', ''' || 'OPERATOR' || ''', ''' || 'ends' || ''', ''' || 'ends' || ''', ''' || 'ends' || ''', 8, 1);');
