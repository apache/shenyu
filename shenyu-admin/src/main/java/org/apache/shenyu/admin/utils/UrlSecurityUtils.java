...
public final class UrlSecurityUtils {
    ...
    private static boolean isPrivateOrInternalIP(final String host) {
        ...
        if (host.startsWith("fc") || host.startsWith("fd") || 
            host.startsWith("fe80") || host.equals("::1")) {
            return true;
        }
        
        // Check for other internal addresses
        if (host.matches("^169\\.254\\..*") || // Link-local
            host.matches("^224\\..*") || // Multicast
            host.matches("^255\\..*")) { // Broadcast
            return true;
        }
    ...
    private static boolean isSensitivePort(final int port) {
        if (port == -1) {
            return false; // Default port, will be 80 or 443
        }
        ...
        Set<Integer> sensitivePorts = new HashSet<>(Arrays.asList(
            ...
            8081,  // Common internal web services
            9090,  // Common internal web services
            9091,  // Common internal web services
            9092,  // Kafka
            9093,  // Kafka
            2375,  // Docker
            2376,  // Docker TLS
