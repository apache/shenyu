...
public class OracleSQLLoader implements InstantiationAwareBeanPostProcessor {
    ...
    private static final String SQL_COMMAND = "INSERT /*+ IGNORE_ROW_ON_DUPKEY_INDEX";
    ...
    protected void init(final DataSourceProperties properties) {
        try {
            // Oracle database must exist to initialize script
            // Because Oracle database creation is very complex, it is not recommended to directly start initialization creation
    ...
    private void execute(final Connection conn) throws IOException, SQLException {
        ...
        try {
            ...
            Reader read = this.fillInfoToSqlFile(Stream.of(AdminConstants.REGX_SHENYU_DICT,
                    AdminConstants.REGX_PLUGIN_HANDLE).toArray(String[]::new),
                    "id");
    ...
    private Reader fillInfoToSqlFile(final String[] regxNames, final String regxValue) throws IOException {
        ...
        while (Objects.nonNull(str = reader.readLine())) {
            ...
            for (String regxName : regxNames) {
                if (str.contains(regxName)) {
                    str = SQLInitUtils.concatCharacter(str, regxValue, AdminConstants.SQL_TYPE_ORACLE);
                    break;
                }
            }
            builder.append(str).append(System.lineSeparator());
