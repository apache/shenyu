...
public final class ApplicationConfigCache {
    ...
    public ReferenceConfig<GenericService> build(final MetaData metaData) {
        ...
        try {
            ApacheDubboParamInfo apacheDubboParamInfo = GsonUtils.getInstance().fromJson(rpcExt, ApacheDubboParamInfo.class);
            if (Objects.nonNull(apacheDubboParamInfo)) {
                if (StringUtils.isNoneBlank(apacheDubboParamInfo.getVersion())) {
                    reference.setVersion(apacheDubboParamInfo.getVersion());
                }
                if (StringUtils.isNoneBlank(apacheDubboParamInfo.getGroup())) {
                    reference.setGroup(apacheDubboParamInfo.getGroup());
                }
                if (StringUtils.isNoneBlank(apacheDubboParamInfo.getLoadbalance())) {
                    final String loadBalance = apacheDubboParamInfo.getLoadbalance();
                ...
                Optional.ofNullable(apacheDubboParamInfo.getTimeout()).ifPresent(reference::setTimeout);
                Optional.ofNullable(apacheDubboParamInfo.getRetries()).ifPresent(reference::setRetries);
            }
        } catch (Exception e) {
