...
package org.dromara.soul.client.core.shutdown;
...
/**
 * Soul client shutdown hook.
 *
 * @author jerbo
 */
@Slf4j
public class SoulClientShutdownHook {

    private static String soulClientShutdownHook = "SoulClientShutdownHook";
    ...
    /**
     * Add soul client shutdown hook.
     *
     * @param result SoulClientRegisterRepository
     * @param props  Properties
     */
    public static void set(final SoulClientRegisterRepository result, final Properties props) {
        String name = soulClientShutdownHook + "-" + hookId.incrementAndGet();
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            result.close();
        }, name));
        log.info("Add hook {}", name);
        SoulClientShutdownHook.props = props;
    }
        ...
        public void run() {
            ...
            while (System.currentTimeMillis() - s < delayOtherHooksExecTime) {
                for (Iterator<Thread> iterator = hooks.keySet().iterator(); iterator.hasNext();) {
                    ...
                    if (hook.getName().startsWith(soulClientShutdownHook)) {
                        continue;
                    }
