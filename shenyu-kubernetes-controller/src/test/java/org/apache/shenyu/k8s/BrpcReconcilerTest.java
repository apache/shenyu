...
/**
 * brpc Ingress Reconciler Test.
 */
public final class BrpcReconcilerTest {
    ...
    public void init() {
        ...
        annotations.put("shenyu.apache.org/plugin-brpc-enabled", "true");
        Map<String, String> labels = new HashMap<>();
        labels.put("shenyu.apache.org/metadata-labels-1", "brpcGetUserService");
        Map<String, String> labelsAnnotations = new HashMap<>();
        labelsAnnotations.put("kubernetes.io/ingress.class", "shenyu");
        labelsAnnotations.put("shenyu.apache.org/plugin-brpc-enabled", "true");
        labelsAnnotations.put("shenyu.apache.org/plugin-brpc-app-name", "brpc");
        labelsAnnotations.put("shenyu.apache.org/plugin-brpc-path", "/getUser");
        labelsAnnotations.put("shenyu.apache.org/plugin-brpc-rpc-type", "brpc");
        labelsAnnotations.put("shenyu.apache.org/plugin-brpc-service-name", "org.apache.shenyu.examples.brpc.api.service.BrpcDemoService");
        labelsAnnotations.put("shenyu.apache.org/plugin-brpc-method-name", "getUser");
        labelsAnnotations.put("shenyu.apache.org/plugin-brpc-params-type", "java.lang.Long");
        labelsAnnotations.put("shenyu.apache.org/plugin-brpc-rpc-expand", "{\"methodInfo\":[{\"methodName\":\"getUser\","
                + "\"paramTypes\":[{\"left\":\"java.lang.Long\",\"right\":\"userId\"}]}],\"host\":brpcHostService,\"port\":8005}");
        V1Service brpcGetUserService = new V1ServiceBuilder().withNewMetadata().withName("brpcGetUserService").withNamespace("mockedNamespace").withAnnotations(labelsAnnotations).endMetadata()
                .withNewSpec().endSpec()
        ...
        when(serviceIndexer.getByKey("mockedNamespace/brpcGetUserService")).thenReturn(brpcGetUserService);
        ...
        V1Endpoints mockedEndpoints = new V1EndpointsBuilder().withKind("Endpoints")
                ...
                .build();
        V1Endpoints brpcHostService = new V1EndpointsBuilder().withNewMetadata().withName("brpcHostService").withNamespace("mockedNamespace").endMetadata()
                .withSubsets(new V1EndpointSubsetBuilder().withAddresses(new V1EndpointAddress().ip("127.0.0.1")).build())
                .build();
        when(endpointsIndexer.getByKey("mockedNamespace/testService")).thenReturn(mockedEndpoints);
        when(endpointsIndexer.getByKey("mockedNamespace/brpcHostService")).thenReturn(brpcHostService);
