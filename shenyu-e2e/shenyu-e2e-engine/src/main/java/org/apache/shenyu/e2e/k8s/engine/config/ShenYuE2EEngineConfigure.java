...
package org.apache.shenyu.e2e.k8s.engine.config;
...
public class ShenYuE2EEngineConfigure {
     ...
     */
    public static ShenYuE2EEngineConfigure fromAnnotation(final ShenYuE2ETest annotation) {
        ShenYuE2ETest.Environment[] environments = annotation.environments();
        ...
        Map<String, List<Environment>> environmentMap = Arrays.stream(environments)
                .collect(Collectors.groupingBy(ShenYuE2ETest.Environment::serviceName));
        ...
        ShenYuE2EEngineConfigure engineConfigure = new ShenYuE2EEngineConfigure();
        engineConfigure.setServiceConfigureMap(map);
        return engineConfigure;
    }
    
    private static Properties toProperties(final ShenYuE2ETest.Parameter[] parameters) {
