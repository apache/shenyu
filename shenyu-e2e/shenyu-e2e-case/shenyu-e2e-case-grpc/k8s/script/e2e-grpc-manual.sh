...
for sync in ${SYNC_ARRAY[@]}; do
  ...
  kubectl apply -f "${PRGDIR}"/shenyu-examples-grpc-disable-register.yml
  sh "$SHENYU_TESTCASE_DIR"/k8s/script/healthcheck.sh http://localhost:30290/grpc/actuator/health
  sleep 10s
  kubectl get pod -o wide

  ## run e2e-test
  ./mvnw -B -f ./shenyu-e2e/pom.xml -pl shenyu-e2e-case/shenyu-e2e-case-grpc -am test -Dtest=org.apache.shenyu.e2e.testcase.grpc.manual.*
