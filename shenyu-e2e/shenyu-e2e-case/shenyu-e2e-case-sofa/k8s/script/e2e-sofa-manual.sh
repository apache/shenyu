...
for sync in ${SYNC_ARRAY[@]}; do
  ...
  kubectl apply -f "${PRGDIR}"/shenyu-examples-sofa-disable-register.yml
  ...
  ## run e2e-test
  ./mvnw -B -f ./shenyu-e2e/pom.xml -pl shenyu-e2e-case/shenyu-e2e-case-sofa -am test -Dtest=org/apache/shenyu/e2e/testcase/sofa/manual/*
