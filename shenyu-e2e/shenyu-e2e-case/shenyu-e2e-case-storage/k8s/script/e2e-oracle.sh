...
# init kubernetes for oracle
shenyuTestCaseDir=$(dirname "$(dirname "$(dirname "$(dirname "$0")")")")
echo "$shenyuTestCaseDir"
bash "$shenyuTestCaseDir"/k8s/script/init/oracle_container_init.sh

curPath=$(readlink -f "$(dirname "$0")")
PRGDIR=$(dirname "$curPath")
echo "$PRGDIR"
kubectl apply -f "${PRGDIR}"/shenyu-deployment-oracle.yml
kubectl apply -f "${PRGDIR}"/shenyu-app-service-oracle.yml
...
sh "${curPath}"/healthcheck.sh oracle http://localhost:31095/actuator/health http://localhost:31195/actuator/health
