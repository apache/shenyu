...
public class ApacheDubboProxyService {
    ...
    public Mono<Object> genericInvoker(final String body, final MetaData metaData, final ServerWebExchange exchange) throws ShenyuException {
        ...
        Pair<ReferenceConfig<GenericService>, String> refComfigPair = ApplicationConfigCache.getInstance().get(metaData.getPath());
        if (StringUtils.isEmpty(refComfigPair.getLeft().getInterface())) {
            ApplicationConfigCache.getInstance().invalidate(metaData.getPath());
            refComfigPair = ApplicationConfigCache.getInstance().initRef(metaData);
        }
        ReferenceConfig<GenericService> reference = refComfigPair.getLeft();
        ...
        if (isProviderSupportAsync(refComfigPair.getRight())) {
    ...
    private boolean isProviderSupportAsync(final String providerVersion) {
