...
public final class ApacheDubboProxyServiceTest {
    ...
    public void genericInvokerTest() throws IllegalAccessException, NoSuchFieldException {
        ...
        Pair<ReferenceConfig, String> refPair = Pair.of(referenceConfig, "2.7.3");
        ...
        ((LoadingCache) field.get(applicationConfigCache)).put(PATH, refPair);
        ...
        applicationConfigCache.invalidateAll();
        Pair<ReferenceConfig, String> refPair1 = Pair.of(referenceConfig, "2.7.2");
        ((LoadingCache) field.get(applicationConfigCache)).put(PATH, refPair1);
        apacheDubboProxyService.genericInvoker("", metaData, exchange);
        verify(genericService, times(1)).$invoke(METHOD_NAME, LEFT, RIGHT);

        applicationConfigCache.invalidateAll();
        Pair<ReferenceConfig, String> refPair2 = Pair.of(referenceConfig, null);
        ((LoadingCache) field.get(applicationConfigCache)).put(PATH, refPair2);
        apacheDubboProxyService.genericInvoker("", metaData, exchange);
        verify(genericService, times(2)).$invoke(METHOD_NAME, LEFT, RIGHT);
    }
