        ...
        if (Objects.nonNull(shenyuTrieNode)) {
            ...
            if (CollectionUtils.isNotEmpty(collection)) {
                ...
                if (collection.size() > 1) {
                    selectorDataPair = matchSelector(exchange, ListUtil.castList(collection, SelectorData.class));
                } else {
    ...
    private RuleData trieMatchRule(final ServerWebExchange exchange, final SelectorData selectorData, final String path) {
        ...
        if (Objects.nonNull(shenyuTrieNode)) {
            ...
            if (CollectionUtils.isNotEmpty(collection)) {
                ...
                if (collection.size() > 1) {
                    ruleDataPair = matchRule(exchange, ListUtil.castList(collection, RuleData.class));
                } else {
