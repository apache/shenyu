...
public class ShenyuToolCallback implements ToolCallback {
    ...
    public String call(@NonNull final String input, final ToolContext toolContext) {
        ...
        try {
            ...
            // Get the original exchange and plugin chain
    ...
    /**
     * Extracts the session ID from the MCP exchange.
     *
     * @param exchange the MCP sync server exchange
     * @return the session ID
     * @throws IllegalStateException if session ID cannot be extracted
     */
    private String extractSessionId(final McpSyncServerExchange exchange) {
        try {
            final String sessionId = McpSessionHelper.getSessionId(exchange);
            if (!StringUtils.hasText(sessionId)) {
                throw new IllegalStateException("Session ID is empty or null");
            }
    ...
    /**
     * Retrieves the original ServerWebExchange for the given session.
     *
     * @param sessionId the MCP session identifier
     * @return the original exchange
     * @throws IllegalStateException if exchange is not found
     */
    private ServerWebExchange getOriginExchange(final String sessionId) {
        ...
        if (Objects.isNull(exchange)) {
            throw new IllegalStateException("No exchange found for session: " + sessionId);
        }
        return exchange;
    }
    ...
    private String executeToolCall(final ServerWebExchange originExchange,
        ...
        try {
            return responseFuture.get(QUICK_TIMEOUT_SECONDS, TimeUnit.SECONDS);
        } catch (Exception e) {
