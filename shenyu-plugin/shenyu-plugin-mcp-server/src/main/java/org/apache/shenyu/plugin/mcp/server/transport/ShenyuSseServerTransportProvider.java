 ...
 * Model Context Protocol (MCP) messages over Server-Sent Events (SSE) using Spring WebFlux.
 * It allows clients to connect via SSE and receive real-time updates for MCP messages.
 */
public class ShenyuSseServerTransportProvider implements McpServerTransportProvider {
     ...
     *
     * @param objectMapper The ObjectMapper to use for JSON serialization/deserialization of MCP messages. Must not be null.
     * @param messageEndpoint The endpoint URI where clients should send their JSON-RPC messages. This endpoint will be communicated to clients during SSE connection setup. Must not be null.
     ...
     * @throws IllegalArgumentException if either parameter is null
     */
    public ShenyuSseServerTransportProvider(final ObjectMapper objectMapper, final String messageEndpoint, final String sseEndpoint) {
        this(objectMapper, DEFAULT_BASE_URL, messageEndpoint, sseEndpoint);
    }
    
    /**
     * Constructs a new WebFlux SSE server transport provider instance with custom base URL.
     *
     * @param objectMapper The ObjectMapper to use for JSON serialization/deserialization of MCP messages. Must not be null.
     * @param baseUrl The base URL for the message endpoint. Must not be null.
     * @param messageEndpoint The endpoint URI where clients should send their JSON-RPC messages. This endpoint will be communicated to clients during SSE connection setup. Must not be null.
     * @param sseEndpoint The SSE endpoint path. Must not be null.
     * @throws IllegalArgumentException if any parameter is null
     */
    public ShenyuSseServerTransportProvider(final ObjectMapper objectMapper, final String baseUrl, final String messageEndpoint, final String sseEndpoint) {
    ...
    /**
     * Returns the RouterFunction that handles incoming requests for this transport.
    ...
    public Mono<ServerResponse> handleSseConnection(final ServerRequest request) {
        ...
        return ServerResponse.ok().contentType(MediaType.TEXT_EVENT_STREAM).body(Flux.<ServerSentEvent<?>>create(sink -> {
            ...
            sink.next(ServerSentEvent.builder().event(ENDPOINT_EVENT_TYPE).data(this.baseUrl + this.messageEndpoint + "?sessionId=" + sessionId).build());
            sink.onCancel(() -> {
    ...
    public Mono<ServerResponse> handleMessage(final ServerRequest request) {
        ...
        if (Objects.isNull(session)) {
            return ServerResponse.status(HttpStatus.NOT_FOUND).bodyValue(new McpError("Session not found: " + request.queryParam("sessionId").get()));
        }
    ...
    /**
     * Builder class for constructing instances of {@link ShenyuSseServerTransportProvider}.
     * This builder allows for setting the ObjectMapper, base URL, message endpoint, and SSE endpoint.
     */
