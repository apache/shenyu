 ...
 * @author xiaoyu(Myth)
 */
public class CommonCacheHandler extends AbstractLocalCacheManager {
     ...
     */
    public void deleteSelectorData(final SelectorData selectorData) {
     ...
     * @param selectorData the selector data
     */
    public void cacheSelectorData(final SelectorData selectorData) {
        if (selectorData.getPluginName().equals(PluginEnum.DIVIDE.getName())) {
        ...
        if (SELECTOR_MAP.containsKey(key)) {
            ...
            List<SelectorData> resultList = existList.stream().filter(r -> !r.getId().equals(selectorData.getId())).collect(Collectors.toList());
